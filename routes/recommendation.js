            ...
            connection.execute("SELECT PHOTOID,AVG_RATING,PIN_COUNT,URL FROM PHOTO WHERE PHOTOID IN " +
                ...
                if(results.length == 0)
                {
                    ...
                    console.log(err);
                        ...
                        {
                            rec_photoids[i] = results[i].PHOTOID;
                ...
                if(results.length == 0)
                {
                    ...
                    console.log(err);
                        ...
                        {  var count = 0;
                            ...
                            if(count==0)
                                {
                                rec_photoids.push(results[j].PHOTOID);
...
function recommend_people(res)
{
    oracle.connect(connectData, function(err, connection) {
        if ( err ) {
            ...
            connection.execute("SELECT USERID, FIRSTNAME, LASTNAME FROM USERS NATURAL JOIN INTEREST " + 
                                "WHERE INTERESTVALUE IN " + 
                                    "(SELECT INTERESTVALUE FROM INTEREST " + 
                                            "WHERE USERID='"+userid+"') AND USERID <> '"+userid+"' AND ROWNUM<10", 
                       [], 
                       function(err, results) {
                if(results.length == 0)
                {
